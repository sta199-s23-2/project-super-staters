...
```
We filtered for songs under 10 minutes because there were several outliers. For rap, there was no relation on average between duration and popularity as the linear fit line was flat. For the other three genres, there was a positive relationship between duration and popularity. Thus, we predict that longer songs are on average more popular for country, pop and rock. 
...
Country:
$\widehat{song.hotttness} = -0.371 -0.000534 * song.tempo$

Pop:
$\widehat{song.hotttness} = -0.371 - 0.216 + (-0.000534 + 0.00303) * song.tempo = -0.587 + 0.002496* song.tempo$

Rap:
$\widehat{song.hotttness} = -0.371 + 0.475 + (-0.000534 -0.00113) * song.tempo = 0.104 - 0.001664 * song.tempo$

Rock:
$\widehat{song.hotttness} = -0.371 + 0.301 + (-0.000534 + 0.000115) * song.tempo = -0.07 - 0.000419 * song.tempo$
...
Based on our interaction models and the AIC and adjusted r.squared values, year and song popularity had the lowest AIC, making it the best fit model of our song attributes. It is difficult to compared which coefficient was the highest due to the difference in units of our attributes. So, based on AIC how recently a song was released (song.year) has the strongest correlation to popularity. 
